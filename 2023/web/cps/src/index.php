...
if (isset($_COOKIE["token"])) {
    $stmt = $mysqli->prepare("SELECT tokens.username, users.password FROM tokens RIGHT JOIN users ON tokens.username = users.username WHERE tokens.token = ?");
    ...
    $message = '<p>welcome, ' . substr($row["username"], 0, 5) . '. your password is ' . $row["password"] . '. <a href="logout.php">log out</a></p>';
} else {
    $message = '<a href="register.php">register</a>
