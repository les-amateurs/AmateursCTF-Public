...
class Hook(pickle._Unpickler):
    def find_class(self, module, name):
        print(f'find_class: {module}.{name}')
        return super().find_class(module, name)
    
    # see patch on line 1590 of pickle.py for reduce and also on load_list for the list
...
print(Hook(io.BytesIO(larry)).load())
# actual solve below

import random

random.seed('five nights as freddy')
mapping = list(range(59))
random.shuffle(mapping)

xor = random.randbytes(59)
...
dec = [a^b for a, b in zip(xor, enc)]
dec = [dec[mapping.index(i)] for i in range(59)]
